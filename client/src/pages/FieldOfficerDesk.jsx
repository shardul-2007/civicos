...
export default function FieldOfficerDesk() {
  ...
  const loadOfficerQueue = useCallback(async () => {
    ...
    try {
      ...
      const msg = err.response?.data?.message || 'Failed to load officer queue. Check server connection.';
      setError(msg);
    } finally {
      setLoading(false);
    }
  ...
  const handleAction = async (complaintId, newStatus, noteText) => {
    ...
    try {
      ...
      if (res.data.success) {
        ...
        setComplaints((prev) =>
          prev.map((c) => (c._id === complaintId ? { ...c, ...res.data.data, status: newStatus } : c))
        );
      }
    } catch (err) {
      // Rollback
      await loadOfficerQueue();
      showToast(err.response?.data?.message || 'Status update failed. Please try again.', 'error');
    } finally {
    ...
    <div style={{ background: 'var(--bg-app)', minHeight: '100vh', padding: '1.5rem 1rem', color: 'var(--text-primary)' }}>
      <div style={{ maxWidth: '900px', margin: '0 auto' }}>
        ...
        <StateWrapper
          loading={loading}
          error={error}
          empty={!loading && !error && filteredComplaints.length === 0}
          emptyTitle="No complaints in this queue"
          emptyMessage={filterStatus === 'ALL' ? 'No complaints have been submitted yet.' : `No complaints with filter "${filterStatus}".`}
          onRetry={loadOfficerQueue}
          loadingLabel="Loading live municipal field queue..."
          minHeight="30vh"
        >
                ...
                >
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '0.5rem', flexWrap: 'wrap', gap: '0.5rem' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', flexWrap: 'wrap' }}>
                      ...
                      {c.safetyRisk && (
                        <span className="badge badge-critical" style={{ fontSize: '0.65rem' }}>⚠ Safety Risk</span>
                      )}
                    </div>
